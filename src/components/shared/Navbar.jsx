                ...
                <div className={`lg:flex lg:items-center lg:w-auto w-full ${this.state.toggle ? '' : 'hidden'}`}>
                    <nav>
                        <ul className="lg:flex items-center justify-between text-base pt-4 lg:pt-0">
                            { menuItems.map(item => (
                                <li key={item.id}><Link to={item.path} className="lg:px-3 px-2 py-2 block font-medium rounded border-b-2 border-transparent hover:bg-gray-200 uppercase" >{item.title}</Link></li>
                            ))}
                        </ul>
                    </nav>
                </div>
